...
app.controller('JobListController', function ($scope, $resource, $stateParams, $modal, $state) {
    ...
    $scope.query = function (page, filter, state) {
        var $com = $resource("/job/admin/jobs/list/:page?filter=:filter&state=:state", {page: '@page'});
        if (!page) {
            page = 1;
        } else {
            page = parseInt(page);
        ...
        if ($scope.operate == "1") {
            ...
            if (ids.length > 0) {
                ...
                modalInstance.result.then(function () {
                    var $com = $resource("/job/admin/jobs/delete", null, {
                        'delete': {method: 'POST'}
                    });
...
app.controller('JobDetailController', function ($rootScope, $scope, $resource, $stateParams, $state, $modal, $http) {
    ...
    if ($scope.edit_mode) {
        var $com = $resource("/job/admin/jobs/detail/:id", {id: '@id'});
        ...
        $scope.companies = [];
        //获取所有的公司
        $http.get("/user/admin/companies/list/0")
            .success(function (d) {
    ...
    $scope.submit = function () {
        ...
        if ($scope.edit_mode) {
            var $com = $resource("/job/admin/jobs/update/:id", {id: '@id'}, {
                'update': {method: 'POST'}
            });
            ...
            var $com = $resource("/job/admin/jobs/create");
