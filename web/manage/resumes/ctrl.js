...
app.controller('ResumeListController', function ($scope, $resource, $stateParams, $modal, $state, $http) {
    //获取院系信息
    ...
    //查询
    $scope.query = function (page, filter, grade, salary, school) {
        var $com = $resource("/job/admin/resumes/list/:page?filter=:filter&grade=:grade&salary=:salary&school=:school", {page: '@page'});
        if (!page) {
            page = 1;
        } else {
            page = parseInt(page);
        }
        $com.get({page: page, filter: filter, grade: grade, salary: salary, school: school}, function (data) {
    ...
    $scope.search = function () {
        $scope.query(1, $scope.search_context, $scope.grade, $scope.salary, $scope.school);
    };
        ...
        if ($scope.operate == "1") {
            ...
            if (ids.length > 0) {
                ...
                modalInstance.result.then(function () {
                    var $com = $resource("/job/admin/resumes/delete", null, {
                        'delete': {method: 'POST'}
                    });
...
app.controller('ResumeDetailController', function ($rootScope, $scope, $resource, $stateParams, $state, $modal, $http) {
    ...
    if ($scope.edit_mode) {
        var $com = $resource("/job/admin/resumes/detail/:id", {id: '@id'});
        ...
        //获取所有用户
        $http.get("/user/admin/users/list/0")
            .success(function (d) {
                $scope.users = d.rows;
            });
    }
    ...
    $scope.submit = function () {
        ...
        if ($scope.edit_mode) {
            var $com = $resource("/job/admin/resumes/update/:id", {id: '@id'}, {
                'update': {method: 'POST'}
            });
            ...
            var $com = $resource("/job/admin/resumes/create");
