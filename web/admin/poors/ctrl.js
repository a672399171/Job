...
app.controller('PoorListController', function ($scope, $resource, $stateParams, $modal, $state) {
    ...
    $scope.query = function (page, filter) {
        var $com = $resource($scope.app.host + "/user/admin/poors/list/:page", {page: '@page'});
        if (!page) {
            page = 1;
        } else {
            page = parseInt(page);
        ...
        if ($scope.operate == "1") {
            ...
            if (ids.length > 0) {
                ...
                modalInstance.result.then(function () {
                    var $com = $resource($scope.app.host + "/user/admin/poors/delete", null, {
                        'delete': {method: 'POST'}
                    });
...
app.controller('PoorDetailController', function ($rootScope, $scope, $resource, $stateParams, $state, $modal, $http) {
    ...
    if ($scope.edit_mode) {
        var $com = $resource($scope.app.host + "/user/admin/poors/detail/:id", {id: '@id'});
    ...
    $scope.submit = function () {
        ...
        //发送请求
        $http.post($scope.app.host + "/user/admin/poors/saveOrUpdatePoor.do", $scope.data)
    ...
    $scope.auth = function (u_id, status) {
        $http.post($scope.app.host + "/user/admin/auth.do", {
            u_id: u_id,
            status: status
        }).then(function (response) {
            $state.go('app.poors.list');
